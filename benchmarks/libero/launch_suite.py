...
def _resolve_worker_task_choice() -> str:
    ...
    if task_choice is None or str(task_choice).strip() == "":
        raise ValueError(
            "Hydra task choice is empty. Please pass task=libero."
        )
...
def run_evaluation(
    *,
    ...
    eval_entrypoint: str = "benchmarks/libero/evaluate_task.py",
    summary_entrypoint: str = "benchmarks/libero/summarize.py",
    trial_plan: str | None = None,
) -> None:
    script_path = Path("benchmarks/libero/scheduler.sh")
    ...
    trial_plan = trial_plan or f"{num_trials} per task"
    ...
    env.update(
        {
            ...
            "SUMMARY_ENTRYPOINT": summary_entrypoint,
        }
    )
    ...
    print(f"Trial plan: {trial_plan}")
    print(f"Max tasks per GPU: {max_tasks_per_gpu}")
    print(f"Output directory: {output_dir}")
    print(f"Evaluation entrypoint: {eval_entrypoint}")
    if extra_args:
