...
from benchmarks.libero.action_queue import ActionEnsembler
...
def _denormalize_action(action: torch.Tensor, processor: Vid2WAMProcessor) -> np.ndarray:
    if action.ndim == 2:
...
def _load_task_init_states(task_suite, task_id: int):
    """Load official LIBERO init states with pickle support."""
...
def run_single_episode(
    ...
    *,
    ...
    pbar = tqdm(
        ...
        disable=not bool(cfg.EVALUATION.get("show_episode_progress", False)),
    )
...
@hydra.main(version_base="1.3", config_path="../../settings", config_name="libero.yaml")
def eval_single_process(cfg: DictConfig):
    ...
    if env_num != 1:
        raise ValueError(
            "Only env_num=1 is supported in evaluate_task.py. "
            "Use launch_suite.py/scheduler.sh for multi-GPU task parallelism."
        )
    ...
    initial_states = _load_task_init_states(task_suite, int(cfg.EVALUATION.task_id))
