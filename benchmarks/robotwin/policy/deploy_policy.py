...
from vid2wam.evaluation.prompts import DEFAULT_PROMPT
...
def _resolve_sim_cfg_name(sim_cfg_path: Optional[str], sim_cfg_name: Optional[str]) -> str:
    settings_root = (PROJECT_ROOT / "settings").resolve()
    if not _is_none_like(sim_cfg_path):
        cfg_path = Path(str(sim_cfg_path)).expanduser().resolve()
        try:
            relative = cfg_path.relative_to(settings_root)
        except ValueError as exc:
            raise ValueError(
                f"`sim_cfg_path` must be under {settings_root}, got: {cfg_path}"
            ) from exc
        return relative.as_posix()

    if _is_none_like(sim_cfg_name):
        return "robotwin.yaml"
...
def _compose_sim_cfg(
    ...
    with initialize_config_dir(version_base="1.3", config_dir=str(settings_root)):
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        self.processor: Vid2WAMProcessor = instantiate(processor_cfg).eval()
...
class RoboTwinEnvironmentTestPolicy:
    """Minimal policy used to validate RoboTwin without constructing Vid2WAM."""

    def __init__(self, checkpoint_path: str, max_steps: int) -> None:
        checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
        if not isinstance(checkpoint, Mapping) or len(checkpoint) != 0:
            raise ValueError(
                "RoboTwin environment test requires an empty mapping checkpoint created "
                "with `torch.save({}, path)`."
            )
        self.max_steps = int(max_steps)
        if self.max_steps <= 0:
            raise ValueError("`environment_test_max_steps` must be positive.")

    def step(self, task_env, observation: Optional[Dict[str, Any]]) -> None:
        if observation is None:
            raise ValueError("RoboTwin environment test requires an observation.")

        obs_data = observation.get("observation", {})
        for camera_name in ("head_camera", "left_camera", "right_camera"):
            rgb = np.asarray(obs_data.get(camera_name, {}).get("rgb"))
            if rgb.ndim != 3 or rgb.shape[-1] != 3:
                raise ValueError(
                    f"Invalid {camera_name} RGB observation shape: {rgb.shape}"
                )
        ...
        task_env.step_lim = min(int(task_env.step_lim), self.max_steps)
        task_env.take_action(state.copy(), action_type="qpos")
...
def get_model(usr_args: Dict[str, Any]):
    ...
    environment_test = _parse_bool(usr_args.get("environment_test", False))
    if environment_test:
        max_steps = int(usr_args.get("environment_test_max_steps", 5))
        return RoboTwinEnvironmentTestPolicy(
