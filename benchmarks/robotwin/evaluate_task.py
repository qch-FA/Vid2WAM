...
@hydra.main(version_base="1.3", config_path="../../settings", config_name="robotwin.yaml")
def main(cfg: DictConfig):
    ...
    policy_source_dir = (PROJECT_ROOT / "benchmarks" / "robotwin" / "policy").resolve()
    ...
    sim_cfg_path = (PROJECT_ROOT / "settings" / "robotwin.yaml").resolve()
    sim_task = HydraConfig.get().runtime.choices.get("task")

    environment_test = _parse_bool(cfg.EVALUATION.get("environment_test", False))
    dataset_stats_path = None
    if not environment_test:
    ...
    _append_override(overrides, "environment_test", environment_test)
    _append_override(
        overrides,
        "environment_test_max_steps",
        cfg.EVALUATION.environment_test_max_steps,
    )
    ...
    _append_override(
        overrides,
        "dataset_stats_path",
        str(dataset_stats_path) if dataset_stats_path is not None else None,
    )
    ...
    checkpoint_base_path = _resolve_optional_path(
        cfg.EVALUATION.get("checkpoint_base_path"), base=PROJECT_ROOT
    )
    if checkpoint_base_path is not None:
        if not checkpoint_base_path.exists():
            raise FileNotFoundError(
                f"`EVALUATION.checkpoint_base_path` not found: {checkpoint_base_path}. "
                "Point it at the local checkpoints dir (containing <model_id>/<files>), "
                "or set it to null to use the loader default."
            )
        env["DIFFSYNTH_MODEL_BASE_PATH"] = str(checkpoint_base_path)

    skip_download_raw = cfg.EVALUATION.get("skip_download", False)
    if isinstance(skip_download_raw, str):
        skip_download = skip_download_raw.strip().lower() in {"1", "true", "yes", "y"}
    else:
        skip_download = bool(skip_download_raw)
    if skip_download:
