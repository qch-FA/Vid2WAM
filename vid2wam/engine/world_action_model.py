...
from vid2wam.wan.flow_scheduler import WanContinuousFlowMatchScheduler

logger = get_logger(__name__)


class Vid2WAM(torch.nn.Module):
    ...
    def from_wan22_pretrained(
        ...
        if video_dit_config is None:
            raise ValueError("`video_dit_config` is required for Vid2WAM.from_wan22_pretrained().")
        if "text_dim" not in video_dit_config:
            raise ValueError("`video_dit_config['text_dim']` is required for Vid2WAM.")
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        if "context" not in sample or "context_mask" not in sample:
            raise ValueError(
                "Vid2WAM requires `sample['context']` and `sample['context_mask']`."
            )
        ...
        if "action" not in sample:
            raise ValueError("`sample['action']` is required for this model path.")
    ...
    def infer_action(
        ...
        use_input_image = input_image is not None
        use_input_latents = input_latents is not None
        if use_input_image and use_input_latents:
            raise ValueError("`input_image` and `input_latents` are mutually exclusive.")
        if not use_input_image and not use_input_latents:
            raise ValueError("Either `input_image` or `input_latents` must be provided.")
        ...
        if use_input_image:
            ...
            # Optional pre-encoded VAE first-frame latent used to bypass image re-encoding.
            if input_latents.ndim == 4:
                input_latents = input_latents.unsqueeze(0)
            if input_latents.ndim != 5 or input_latents.shape[0] != 1 or input_latents.shape[2] != 1:
                raise ValueError(
                    f"`input_latents` must have shape [1,C,1,H,W] or [C,1,H,W], got {tuple(input_latents.shape)}"
                )
        if proprio is not None:
            if self.proprio_dim is None:
                raise ValueError("`proprio` was provided but `proprio_dim=None` so `proprio_encoder` is disabled.")
        ...
        if use_input_image:
            ...
            first_frame_latents = input_latents.to(device=self.device, dtype=self.torch_dtype)
        fuse_flag = bool(getattr(self.video_expert, "fuse_vae_embedding_in_latents", False))
